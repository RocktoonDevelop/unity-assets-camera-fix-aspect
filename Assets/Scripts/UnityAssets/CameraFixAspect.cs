...
namespace Rocktoon.UnityAssets {
    public class CameraFixAspect : MonoBehaviour {

        [SerializeField] private float xAspect = 9.0f;
        [SerializeField] private float yAspect = 16.0f;


        void Awake() {
            Camera camera = this.GetComponent<Camera>();
            Rect rect = this.calcAspect(this.xAspect, this.yAspect);
            camera.rect = rect;
        }
